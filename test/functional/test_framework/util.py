...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s ZIP too low! (Should be %s ZIP)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s ZIP too high! (Should be %s ZIP)" % (str(fee), str(target_fee)))
...
def initialize_datadir(dirname, n, chain):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "zip.conf"), n=n, chain=chain)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "zip.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "zip.conf")):
        with open(os.path.join(datadir, "zip.conf"), 'r', encoding='utf8') as f:
            for line in f:
