...
static bool AppInit(int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/zip.conf are parsed in qt/bitcoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  zipd [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see zipd -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for zipd but not for the GUI so do this here
