...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Send 210 ZIP from 0 to 2 using sendtoaddress call.
        ...
        # node0 should end up with 1000 ZIP in block rewards plus fees, but
        ...
        # Send 100 ZIP with subtract fee from amount
        ...
        # Sendmany 100 ZIP with subtract fee from amount
        ...
        # Sendmany with explicit fee (ZIP/kB)
        ...
        if not self.options.descriptors:
            ...
            # This will raise an exception for attempting to get the private key of an invalid 6Zip address
            assert_raises_rpc_error(-5, "Invalid 6Zip address", self.nodes[0].dumpprivkey, "invalid")

            # This will raise an exception for attempting to set a label for an invalid 6Zip address
            assert_raises_rpc_error(-5, "Invalid 6Zip address", self.nodes[0].setlabel, "invalid address", "label")

            # This will raise an exception for importing an invalid address
            assert_raises_rpc_error(-5, "Invalid 6Zip address or script", self.nodes[0].importaddress, "invalid")
            ...
            # send with explicit 6zip/kb fee
            self.log.info("test explicit fee (sendtoaddress as 6zip/kb)")
            self.nodes[0].generate(1)
            self.sync_all(self.nodes[0:3])
