...
The Zip Core daemon can be configured to act as a trusted "border
router", implementing the zip wire protocol and relay, making
...
The ZeroMQ feature in Zip Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building zipd:
...
For instance:
    ...
    $ zipd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/zipd.tx.raw \
...
These options can also be provided in zip.conf.
...
From the perspective of zipd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into zipd directly. Furthermore, no information is
...
using. Zipd appends an up-counting sequence number to each
