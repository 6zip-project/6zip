...
def get_label(pr_num):
    return requests.get(f'https://api.github.com/repos/6zip-online/zip/pulls/{pr_num}').json()['head']['label']
...
def main():
    ...
    for conflict in conflictPrs:
        ...
        r = requests.get(f'https://api.github.com/repos/6zip-online/zip/pulls/{this_pr_num}')
        ...
        r = requests.get(f'https://github.com/6zip-online/zip/branches/pre_mergeable/{our_pr_label}...{get_label(this_pr_num)}')
