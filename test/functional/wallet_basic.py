...
class WalletTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # This will raise an exception for attempting to get the private key of an invalid Zip address
        assert_raises_rpc_error(-5, "Invalid Zip address", self.nodes[0].dumpprivkey, "invalid")

        # This will raise an exception for attempting to set a label for an invalid Zip address
        assert_raises_rpc_error(-5, "Invalid Zip address", self.nodes[0].setlabel, "invalid address", "label")

        # This will raise an exception for importing an invalid address
        assert_raises_rpc_error(-5, "Invalid Zip address or script", self.nodes[0].importaddress, "invalid")
        ...
        # send with explicit zip/kb fee
        self.log.info("test explicit fee (sendtoaddress as zip/kb)")
        self.nodes[0].generate(1)
        self.sync_all(self.nodes[0:3])
