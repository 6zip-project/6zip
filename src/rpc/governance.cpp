        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // This command is dangerous because it consumes 5 ZIP irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on zipcentral etc. and do not save them...
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* 6Zip features */
    { "zip",               "getgovernanceinfo",          &getgovernanceinfo,       {} },
    { "zip",               "getsuperblockbudget",        &getsuperblockbudget,     {"index"} },
    { "zip",               "gobject",                    &gobject,                 {"command"} },
    { "zip",               "gobject", "count",           &gobject_count,           {"mode"} },
    { "zip",               "gobject", "deserialize",     &gobject_deserialize,     {"hex_data"} },
    { "zip",               "gobject", "check",           &gobject_check,           {"hex_data"} },
#ifdef ENABLE_WALLET
    { "zip",               "gobject", "prepare",         &gobject_prepare,         {"parent-hash", "revision", "time", "data-hex", "use-IS", "outputHash", "outputIndex"} },
    { "zip",               "gobject", "list-prepared",   &gobject_list_prepared,   {"count"} },
    { "zip",               "gobject", "vote-many",       &gobject_vote_many,       {"governance-hash", "vote", "vote-outcome"} },
    { "zip",               "gobject", "vote-alias",      &gobject_vote_alias,      {"governance-hash", "vote", "vote-outcome", "protx-hash"} },
#endif
    { "zip",               "gobject", "submit",          &gobject_submit,          {"parent-hash", "revision", "time", "data-hex", "fee-txid"} },
    { "zip",               "gobject", "list",            &gobject_list,            {"signal", "type"} },
    { "zip",               "gobject", "diff",            &gobject_diff,            {"signal", "type"} },
    { "zip",               "gobject", "get",             &gobject_get,             {"governance-hash"} },
    { "zip",               "gobject", "getcurrentvotes", &gobject_getcurrentvotes, {"governance-hash", "txid", "vout"} },
    { "zip",               "voteraw",                    &voteraw,                 {"mn-collateral-tx-hash","mn-collateral-tx-index","governance-hash","vote-signal","vote-outcome","time","vote-sig"} },
