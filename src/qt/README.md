This directory contains the source code for the 6Zip Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the 6Zip UI.
...
- ClientModel is used by the main application `zipgui` and several models like `peertablemodel`.
...
#### paymentserver.(h/cpp)
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a zip:... link from a browser).
...
**Note:** Do not change `local/zip_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `zip-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `zip-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `zip-qt`
9. You're all set! Start developing, building, and debugging the 6Zip Core GUI
