This directory contains integration tests that test zipd and its
...
zipd and zip-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the zip utilities, currently only
zip-tx.
...
Before tests can be run locally, Zip Core must be built.  See the [building instructions](/doc#building) for help.
...
Many Zip specific tests require zip_hash. To install it:

- Clone the repo `git clone https://github.com/6zip-online/zip_hash`
- Install zip_hash `cd zip_hash && pip3 install -r requirements.txt .`
...
The P2P and RPC ports used by the zipd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another zipd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its zipd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other zipd processes are running.

On linux, the test framework will warn if there is another
zipd process running when the tests are started.

If there are zombie zipd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
zipd processes running on the system, so should not be used if any non-test
zipd processes are being run.**
...
sure zipd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and zipd `debug.log`s will all be dumped
...
`test_framework.log` and zipd `debug.log`s can be combined into a single
...
call methods that interact with the zipd nodes-under-test.

If further introspection of the zipd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/zipd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/zipd <pid>
```
