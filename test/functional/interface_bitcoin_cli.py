...
# The block reward of coinbaseoutput.nValue (500) ZIP/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 500 ZIP/block.
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `zip-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and zip-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and zip-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (6ZIP/kB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (6ZIP/kB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (500 ZIP) to the default wallet.
