...
- [Development guidelines](#development-guidelines)
    - [General Zip Core](#general-zip-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Zip Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
Zip Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/6zip-online/zip/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_zip
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_zip --log_level=test_suite
$ valgrind -v --leak-check=full src/zipd -printtoconsole
...
# A coverage report will now be accessible at `./test_zip.coverage/index.html`.
```
...
To profile a running zipd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep zipd` -- sleep 60
```
...
Zip Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`zipd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
...
However, in open source software such as Zip Core, where everyone uses
...
pay attention to for reviewers of Zip Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Zip Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
be sent upstream, but bugfixes may also be prudent to PR against Zip Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `zip-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `zip-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Zip Core, the wallet was always
...
- Use *invalid* Zip addresses (e.g. in the constant array `EXAMPLE_ADDRESS`) for
