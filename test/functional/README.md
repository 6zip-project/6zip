...
- Set the `self.setup_clean_chain` variable in `set_test_params()` to `True` to
  ...
  mature block subsidies (25x500=12500 ZIP) in its wallet. Using them is much more
...
with the zipd(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a zipd.  `P2PInterface`
...
Test objects for interacting with a zipd node over the p2p interface.
