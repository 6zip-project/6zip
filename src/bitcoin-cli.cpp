...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to zipd). This changes the RPC endpoint used, e.g. http://127.0.0.1:28081/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires zipd server to be running v21.0 and up");
        }
...
public:
    ...
    const std::string m_help_doc{
        ...
        "> zip-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> zip-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title zip-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "See this help\n"
