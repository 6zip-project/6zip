...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave zipds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop zipds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use zip-cli instead of RPC for all commands")
        parser.add_argument("--zipd-arg", dest="zipd_extra_args", default=[], action="append",
                            help="Pass extra args to all zipd instances")
    ...
    def setup(self):
        ...
        fname_bitcoind = os.path.join(
            ...
            "src",
            "zipd" + config["environment"]["EXEEXT"],
        )
        fname_bitcoincli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "zip-cli" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoind = os.getenv("BITCOIND", default=fname_bitcoind)
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=fname_bitcoincli)

        self.extra_args_from_options = self.options.zipd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: zipds were not stopped and may still be running")
    ...
    def setup_nodes(self):
        ...
        """If this method is updated - backport changes to  ZipTestFramework.setup_nodes"""
        extra_args = None
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'zipd', self.options.bitcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'zip-cli', self.options.bitcoincli) for v in versions]
    ...
    def dynamically_initialize_datadir(self, node_p2p_port, node_rpc_port):
        ...
        with open(os.path.join(new_data_dir, "zip.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as zipd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in zip.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if zipd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("zipd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if zip-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("zip-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if zip-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("zip-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether zip-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether zip-wallet was compiled."""
...
class ZipTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_zip_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, evo_count=0):
        ...
        self.set_zip_dip8_activation(200)
        ...
        # This is nRequestTimeout in zip-q-recovery thread
    ...
    def set_zip_dip8_activation(self, activate_after_block):
    ...
    def set_zip_llmq_test_params(self, llmq_size, llmq_threshold):
