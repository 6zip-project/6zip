...
class ToolWalletTest(BitcoinTestFramework):
    ...
    def zip_wallet_process(self, *args):
        binary = self.config["environment"]["BUILDDIR"] + '/src/zip-wallet' + self.config["environment"]["EXEEXT"]
    ...
    def assert_raises_tool_error(self, error, *args):
        p = self.zip_wallet_process(*args)
    ...
    def test_invalid_tool_commands_and_args(self):
        ...
        # `zip-wallet help` raises an error. Use `zip-wallet -help`.
        ...
        self.assert_raises_tool_error('No method provided. Run `zip-wallet -help` for valid methods.')
        self.assert_raises_tool_error('Wallet name must be provided when creating a new wallet.', 'create')
        locked_dir = os.path.join(self.options.tmpdir, "node0", self.chain, "wallets")
        error = 'Error initializing wallet database environment "{}"!'.format(locked_dir)
        if self.options.descriptors:
            error = "SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another zipd?"
