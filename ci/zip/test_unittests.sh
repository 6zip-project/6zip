...
cd build-ci/zipcore-$BUILD_TARGET
...
if [ "$DIRECT_WINE_EXEC_TESTS" = "true" ]; then
  ...
  wine ./src/test/test_zip.exe
else
  if [ "$RUN_UNIT_TESTS_SEQUENTIAL" = "true" ]; then
    ${TEST_RUNNER_ENV} ./src/test/test_zip --catch_system_errors=no -l test_suite
  else
